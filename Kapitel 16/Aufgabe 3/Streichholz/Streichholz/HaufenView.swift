...
class HaufenView: UIView {
    ...
    func entferneHoelzer(anzahl: Int) {
        ...
        for var i=1; i<=entferneZahl; i+=1 {
            ...
            // Bestimme die Dauer der Animation als Zufallszahl
            // zwischen 0.5 und 1.5 Sekunden.
            let dauerInZehntelSekunden = arc4random_uniform(11)
            let dauerInSekunden = 0.5 + 0.1*Double(dauerInZehntelSekunden)

            // Löse die Animation aus, die Länge ist dabei die
            // Zfallszahl dauerInSekunden.
            UIView.animateWithDuration(dauerInSekunden,
                                       animations: { () in
